...
from RandomForestIndirectAddition import RandomForestIndirectAddition
import gc

"""Anomaly Detection using RandomForest and SMOTE over sampling.

This script reads credit card data and implements anomaly detection using Random Forest. The Random Forest consists of
Decition Trees that are trained in all anomaly data and a part of normal data.The model's performance is measured with
KFold Cross Validation by calculating accuracy, recall, precision and F1 score. The KFold Cross Validation is implementing
at the same time at both anomaly and normal data.

"""
#Initialized parameters
kFold = 10
maxNumberOfSplits = 400
...
for normalIndexes, anomalyIndexes in zip(kf.split(normalArray), kf.split(anomalyArray)):
    ...
    randomForest = RandomForestIndirectAddition(x_train, y_train, maxNumberOfSplits=maxNumberOfSplits)
